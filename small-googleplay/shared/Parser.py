...
class XPath:
    ...
    xPaths = {
        ...
        "ShortDescription": "//meta[@name='description']/@content",
        ...
        "Installations": "//div[@class='content' and @itemprop='numDownloads']/text()",
        ...
        "Size" : "//div[@class='meta-info']/div[@itemprop='fileSize']/text()"
    }
...
class parser:
    ...
    def parse_app_data(self, html):
        ...
        coverimg_url = self.extract_node_text(html_map, 'CoverImgUrl')
        if 'https:' not in coverimg_url:
            coverimg_url = 'https:' + coverimg_url
        app_data['CoverImgUrl'] = coverimg_url
        screenshots_list = self.extract_node_text(html_map, 'Screenshots', True)
        screenshots_list_size = len(screenshots_list)
        for i in range(screenshots_list_size):
            if 'https:' not in screenshots_list[i]:
                screenshots_list[i] = 'https:' + screenshots_list[i]
        app_data['Screenshots'] = screenshots_list
	...
	if self.extract_node_text(html_map, 'Description', True) == None:
            app_data['Description'] = ""
        else:
            app_data['Description'] = "\n".join(self.extract_node_text(html_map, 'Description', True))
        if self.extract_node_text(html_map, 'ShortDescription', True) == None:
            app_data['ShortDescription'] = ""
        else:
            app_data['ShortDescription'] = "\n".join(self.extract_node_text(html_map,'ShortDescription', True))
	if self.extract_node_text(html_map, 'WhatsNew', True) == None:
            app_data['WhatsNew'] = ""
        else:
        ...
        tmp_value = self.extract_node_text(html_map, 'Installations')
        if tmp_value != None:
          tmp_value = tmp_value.replace(',','').split(' ')
	else:
	  tmp_value = ['1',',','1']
        app_data['Installations'] = (int(tmp_value[0]) + int(tmp_value[2])) /2
